...
func (r *KubevipCPConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("KubevipCPConfig", req.NamespacedName)

	log.Info("Start reconciliation for KubevipCPConfig")
	...
	if err := r.Client.Get(ctx, req.NamespacedName, kvcpConfig); err != nil {
		if apierrors.IsNotFound(err) {
			log.Info("KubevipCPConfig resource not found")
			return ctrl.Result{}, nil
		}
		log.Error(err, "Unable to fetch KubevipCPConfig resource")
	...
	cluster, err := r.getOwnerCluster(ctx, kvcpConfig)
	if cluster == nil {
	...
	if res, err := r.reconcileKubevipCPConfig(ctx, kvcpConfig, cluster); err != nil {
		log.Error(err, "Failed to reconcile KubevipCPConfig")
		return res, err
	}
	return ctrl.Result{}, nil
}
